...
%
function COMMIT_PrecomputeRotationMatrices( lmax )
    if nargin < 1, lmax = 12; end
    global COMMIT_data_path

    fprintf( '\n-> Precomputing rotation matrices for l_max=%d:\n', lmax );

    filename = fullfile(COMMIT_data_path,sprintf('AUX_matrices__lmax=%d.mat',lmax) );
    ...
    [colatitude, longitude] = COMMIT_Cart2sphere( grad500(:,1), grad500(:,2), grad500(:,3) );
    Ylm = COMMIT_CreateYlm( lmax, colatitude, longitude );
    ...
    for ox = 0:180
    for oy = 0:180
        Ylm_rot{ox+1,oy+1} = COMMIT_CreateYlm( lmax, ox/180.0*pi, oy/180.0*pi );
    end
    end
