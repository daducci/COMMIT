...
% ----------
% filenameHR : string
%   Filename for the generated high-resolution scheme
% bscale : float
%   If needed, apply a scaling to the b-values (default : 1)

function COMMIT_CreateHighResolutionScheme( filenameHR, bscale )
    global CONFIG
    ...
    if ~exist( CONFIG.schemeFilename, 'file' ) || isempty(CONFIG.scheme)
        error( '[COMMIT_CreateHighResolutionScheme] Scheme file "%s" not found', CONFIG.schemeFilename )
    end
    ...
    if CONFIG.scheme.version == 0
        ...
        for d = 1:size(schemeHR,1)
            fprintf(fidCAMINO,'%15e %15e %15e %15e\n', schemeHR(d,1),schemeHR(d,2),schemeHR(d,3), bscale*bs(d) ); % need to scale the b-values because of different conventions
        end
    else
