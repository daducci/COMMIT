...
xNorm0 = NaN(min(maxIts,10000),1);
xNorm1 = NaN(min(maxIts,10000),1);
rNorm2 = NaN(min(maxIts,10000),1);
...
while 1
    ...
    % Update history info
	if logLevel >= 3
		xNorm0(iter+1) = nnz( sum(reshape( x(1:A.nR*A.nF), A.nF, A.nR ),2) );
	end
	...
	if ( logLevel >= 3 )
		plotResult(iter, maxIts, A, x, rNorm2, sigma, xNorm1, xNorm0 )
	end
...
function plotResult(iter, maxIts, A, x, rNorm2, sigma, xNorm1, xNorm0 )
	sfigure(1); clf
	...
	subplot(4,1,2), plot( rNorm2 )
	if ~isempty(sigma)
		hold on, plot([1 maxIts],sigma*[1 1], 'm--')
	end
	axis tight;	if ( max(rNorm2)>0 ), ylim([0 max(rNorm2)*1.02]); end
	grid on, title( sprintf('|| Ax - y ||_2 = %.2f',rNorm2(iter+1)) )

	subplot(4,1,3), plot( xNorm1 )
	axis tight, if ( max(xNorm1)>0 ), ylim([0 max(xNorm1)*1.02]); end
	grid on, title('|| x ||_1')

	if A.nF > 0
		subplot(4,1,4), plot( xNorm0 )
		axis tight, ylim([0 A.nF])
		grid on, title( sprintf('|| x_{fibers} ||_0 = %d (%.1f%%)', xNorm0(iter+1),xNorm0(iter+1)/A.nF*100) )
	end
