%
% Load the SPARSE STRUCTURE previously created with "trk2dictionary"
%
% Parameters
% ----------
% folder : string
...
function COMMIT_LoadDictionary( folder )
    ...
    DICTIONARY.IC.v  = uint32(vx)  + CONFIG.dim(1) * ( uint32(vy)  + CONFIG.dim(2) * uint32(vz) );
    clear vx vy vz
    ...
    DICTIONARY.EC.v  = uint32(vx)  + CONFIG.dim(1) * ( uint32(vy)  + CONFIG.dim(2) * uint32(vz) );
    clear vx vy vz
    ...
    DICTIONARY.ISO.v = uint32(vx-1) + CONFIG.dim(1) * ( uint32(vy-1) + CONFIG.dim(2) * uint32(vz-1) );
