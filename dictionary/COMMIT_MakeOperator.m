...
methods
    ...
    function obj = COMMIT_MakeOperator()
    ...
    function v = mtimes( obj, x )
        ...
        if obj.adjoint == 0
            ...
            v = COMMIT_MakeOperator_A( DICTIONARY, KERNELS, x, THREADS );
        else
            % At*y
            if ( obj.size(2) ~= size(x,1) ), error('At.times(): dimensions do not match'); end
            v = COMMIT_MakeOperator_At( DICTIONARY, KERNELS, x, THREADS );
        end
    end
