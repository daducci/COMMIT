...
function COMMIT_SetSolver( solver, optTol, maxIter, verbosity )
    ...
    mex( ...
        ...
        '-outdir', fullfile(COMMIT_code_path,'dictionary'), fullfile(COMMIT_code_path,'dictionary','COMMIT_MakeOperator_A.cpp') ...
    );

    mex( ...
        sprintf('-DnIC=%d',numel(KERNELS.wmr)), sprintf('-DnEC=%d',numel(KERNELS.wmh)), sprintf('-DnISO=%d',numel(KERNELS.iso)), ...
        sprintf('-DnTHREADS=%d',CONFIG.OPTIMIZATION.nTHREADS), '-O', '-silent', ...
        '-outdir', fullfile(COMMIT_code_path,'dictionary'), fullfile(COMMIT_code_path,'dictionary','COMMIT_MakeOperator_At.cpp') ...
    );
